...
namespace Varjo
{
    public class Varjo_SteamVR_Controller
    {
        ...
        public class Device
        {
            ...
            public Varjo_SteamVR_Utils.RigidTransform transform { get { Update(); return new Varjo_SteamVR_Utils.RigidTransform(pose.mDeviceToAbsoluteTracking); } }
            ...
            public void Update()
            {
                if (Time.frameCount != prevFrameCount)
                {
                    ...
                    if (system != null)
                    {
                        valid = system.GetControllerStateWithPose(Varjo_SteamVR_Manager.instance.TrackingSpace, index, ref state, (uint)System.Runtime.InteropServices.Marshal.SizeOf(typeof(VRControllerState_t)), ref pose);
                        UpdateHairTrigger();
                    }
                }
            }
            ...
            int relativeTo = (int)OpenVR.k_unTrackedDeviceIndex_Hmd) // use -1 for absolute tracking space
        {
            ...
            var invXform = ((uint)relativeTo < OpenVR.k_unMaxTrackedDeviceCount) ?
                Input(relativeTo).transform.GetInverse() : Varjo_SteamVR_Utils.RigidTransform.identity;
